...
PSD レイヤー付きキャラクター・PNG/WebP 立ち絵・動画背景・BGM・セリフ・音声・テロップを組み合わせて、1920×1080 の MP4 / MOV を書き出せます。
...
- 配置プリセット (キャラ 1 体の X / Y / 拡大率) の保存・呼出し。表情プリセットとは独立した立ち位置テンプレート
- BGM・背景動画・ビジュアライザ・体の揺れを「シーンごと」と「プロジェクト通し」から選択。BGM をプロジェクト全長でループさせられる
- 1 プロジェクト内に複数シーン。タイムラインのシーンレーンでカット単位に区切り、シーンごとに BGM や背景動画を切り替えられる
- シーンの切れ目にもトランジション（カットと同じ 6 種）。境界をまたいで両シーンが動いたまま切り替わる
- ケンバーンズ (カット尺いっぱいのゆっくりズーム / パン)。背景・前景・キャラを一緒に動かし、セリフとテロップは画面に固定
