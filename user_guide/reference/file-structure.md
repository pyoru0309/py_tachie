...
`cache/` は安全にまるごと削除できます。次回再生 / 書き出し時に必要分だけ再生成されます。アプリ内では **アセット管理 → キャッシュ タブ** から個別 / 共有 / 全体を削除できます (詳細は [素材管理 — キャッシュタブ](./assets.md#キャッシュタブ))。起動時の mtime ベース自動間引きも `全体設定 → キャッシュ` で ON/OFF できます (既定 6 時間)。
