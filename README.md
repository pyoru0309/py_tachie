...
- Python **3.11 以上** (3.12 / 3.14 で動作確認)。**書き出し速度を重視するなら 3.12 または 3.13 を推奨**（下記注記）
...
> **⚠ 書き出し速度と Python バージョンについて**
> 動画書き出しは「ブラウザ → サーバ」を WebSocket で大量のフレームを送る経路を使います。
> このとき `websockets` パッケージの C 拡張 (`websockets.speedups`) が高速化を担いますが、
> **Python 3.14 ではこの拡張の wheel が無い場合があり**、pure-Python にフォールバックして
> 特に Windows で書き出しが本来の 1/4〜1/6 まで遅くなることがあります。
> 書き出し速度が気になる場合は **Python 3.12 / 3.13** での実行を推奨します。
> 現在の構成が高速化拡張を使えているかは、起動ログ、または
> **全体設定 → アップデート → 「実行環境診断」** で確認できます。
...
依存パッケージ (`requirements.txt`) の構成が変わったアップデートでは、**「依存パッケージも再インストールする」** にチェックを入れると、適用後に `pip install -r requirements.txt` を自動実行します（既定オフ）。失敗してもコードの更新自体は完了扱いになるので、その場合は手動で `pip install -r requirements.txt` を実行してください。
